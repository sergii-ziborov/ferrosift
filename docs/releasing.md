...
Cargo works out the order — `ferrosift-model` first, then `ferrosift-core`,
`ferrosift-compat`, `ferrosift-operations`, `ferrosift-pattern`, `ferrosift`,
and `ferrosift-cli`. It waits for the index between them, so a member is never
uploaded before the version it names is resolvable.
