...
use ferrosift_model::{ArgumentKind, ArgumentValue};
...
fn supported_step_maps_exact_alias_arguments_and_flags() {
    ...
    let report = import_recipe(source, &registry).expect("bounded recipe parses");
...
fn operation_alias_lookup_is_case_sensitive_and_never_guessed() {
    ...
    let report = import_recipe(br#"[{"op":"test transform","args":[]}]"#, &registry)
...
fn invalid_step_fields_are_ordered_findings_and_fail_closed() {
    ...
    let report = import_recipe(source, &registry).expect("step-level divergences are reportable");
...
fn argument_count_and_kind_divergences_are_explicit() {
    ...
    let report = import_recipe(source, &registry).expect("argument divergences are reportable");
    let codes: Vec<_> = report.findings.iter().map(|finding| finding.code).collect();
...
fn bytes_lists_and_maps_convert_without_string_coercion() {
    ...
    let report = import_recipe(source, &registry).expect("structured JSON parses");
...
fn strings_are_not_guessed_to_be_bytes() {
    ...
    let report = import_recipe(br#"[{"op":"Bytes","args":["abc"]}]"#, &registry)
...
fn excessive_argument_depth_is_a_stable_finding() {
    ...
    let report = import_recipe(source.as_bytes(), &registry)
